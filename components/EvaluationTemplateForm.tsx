...
export default function EvaluationTemplateForm({
  ...
  const [form, setForm] = useState<EvaluationTemplateFormData>({
    ...
    evaluationPrompt: defaultValues?.evaluationPrompt ?? DEFAULT_EVALUATION_PREAMBLE,
  ...
  const canSubmit = form.name.trim() !== "" && form.criteria.length > 0 && weightOk;

  function addCriterion() {
    if (!newCriterion.criterionName.trim() || !newCriterion.description.trim() || !newCriterion.weight) return;
    setForm({
      ...form,
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="flex flex-wrap gap-3 items-start">
                    <div className="w-48 shrink-0">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          updateCriterion(index, "criterionName", e.target.value)
                        }
          ...
          <Card className="border-dashed border-gray-300 bg-gray-50/50">
            <CardContent className="pt-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-3 items-end">
                <div className="w-48 shrink-0">
                  <Label htmlFor="new-criterion-name" className="text-xs text-gray-500">
                    Nome *
                  </Label>
                  <Input
                    ...
                    onChange={(e) =>
                      setNewCriterion({ ...newCriterion, criterionName: e.target.value })
                    }
                    placeholder="Ex: Corretude"
                    className="mt-1"
                  />
              ...
              <div>
                <Label htmlFor="new-description" className="text-xs text-gray-500">
                  Descrição *
                </Label>
                <Textarea
                  id="new-description"
                ...
                <div key={i} className="flex items-start gap-4 px-4 py-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800">
                      {c.criterionName || <span className="italic text-gray-400">Sem nome</span>}
                    </p>
                    {c.description ? (
                      <p className="text-xs text-gray-500 mt-0.5">{c.description}</p>
                    ) : (
                      <p className="text-xs italic text-gray-400 mt-0.5">Sem descrição</p>
                    )}
                  </div>
      ...
      <Card className="border-gray-200">
        ...
        <CardContent>
          ...
          <button
            type="button"
            onClick={() =>
              setForm({ ...form, evaluationPrompt: DEFAULT_EVALUATION_PREAMBLE })
            }
      ...
      >
        {mode === "create" ? "Criar template de avaliação" : "Salvar alterações"}
      </Button>
    </div>
  );
}
